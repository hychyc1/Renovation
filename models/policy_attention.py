...
class VillagePolicySharedAttn(nn.Module):
    """
    ...
    This version uses self-attention to create the global summary vector,
    replacing the simpler mean operation.
    """

    def __init__(self, cfg: Config):
        super().__init__()
        print("LOAD shared Policy with Attention", flush=True) # CHANGED
        ...
        # === Mode head (DeepSets-style with Attention) ===
        # per-node encoder
        ...
        # NEW: Self-attention layer for global summary
        self.global_summary_attention = nn.MultiheadAttention(
            embed_dim=mode_phi_dim,
            num_heads=4, # A sensible default, can be tuned
            batch_first=True
        )
    ...
    def forward(self, village_embeddings: torch.Tensor):
        ...
        # --- CHANGED: Global summary via self-attention ---
        # 2a) Let nodes attend to each other
        attn_output, _ = self.global_summary_attention(h, h, h) # Q, K, V are all h
        
        # 2b) Pool the attention-infused features to get the global summary
        g = attn_output.mean(dim=1)                     # [B, phi]
        # --- End of change ---
    ...
    # The methods below this point do not need to be changed as they
    # operate on the final `logits_flat` output of the forward pass.
