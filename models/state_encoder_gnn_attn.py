...
class FeatureEncoderGNNAttn(nn.Module):
    """
    GNN-based encoder that incorporates year and area_so_far into both global and village features.
    The global feature now includes an attention-based summary of village features.
    ...
    def __init__(self, cfg: Config):
        ...
        # NEW: Self-attention for pooling village features for the global state
        self.village_attention = nn.MultiheadAttention(
            embed_dim=self.hidden_dim, 
            num_heads=4, 
            batch_first=True # This simplifies the forward pass
        )
        ...
        # CHANGED: Global MLP now takes 4 inputs: grid_pool, village_pool_attn, year, area
        global_mlp_in_dim = 4 * self.hidden_dim
        self.global_mlp   = nn.Sequential(
            nn.Linear(global_mlp_in_dim, self.global_feature_dim),
            nn.ReLU()
        )
    ...
    def forward(self, grid_input, village_data, year, area_so_far):
        ...
        # Batch edges
        total_nodes = H*W + K
        offsets = torch.arange(B, device=device) * total_nodes
        ...
        v = self.village_proj(village_data[...,2].unsqueeze(-1))
        ...
        # Split GNN output into grid and village nodes
        grid_nodes = x[:, :H*W, :]
        village_nodes = x[:, H*W:, :]

        # --- Global Feature Calculation ---
        # 1. Pool grid features (unchanged)
        grid_pool = grid_nodes.mean(dim=1)

        # NEW: 2. Pool village features using self-attention
        # The village nodes attend to each other to produce a context-aware representation
        attn_output, _ = self.village_attention(village_nodes, village_nodes, village_nodes)
        # We then take the mean of the attention-infused village features as the summary
        village_pool_attn = attn_output.mean(dim=1)

        # 3. Embed year and area_so_far (unchanged)
        ...
        # CHANGED: 4. Fuse all features for the global MLP
        gl_in = torch.cat([grid_pool, village_pool_attn, ye, ae], dim=-1)
        global_feats = self.global_mlp(gl_in)

        # --- Village Feature Calculation (unchanged) ---
        ye_v = ye.unsqueeze(1).expand(-1, K, -1)
        ae_v = ae.unsqueeze(1).expand(-1, K, -1)
        v_in = torch.cat([village_nodes, ye_v, ae_v], dim=-1)
