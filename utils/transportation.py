...
class Transportation:
    def __init__(self, n, m, grid_population, extra_grid, mask):
        """
        ...
        """
        self.n, self.m = n, m
        self.mask_flattened = mask.flatten() if mask is not None else None
    ...
    def calc_transport_time(self):
        ...
        if self.mask_flattened is not None:
            # masked_pop = self.population
            avg = (self.mask_flattened * self.population[:self.N_grid] * dis[:self.N_grid]).sum() / (self.mask_flattened * self.population[:self.N_grid]).sum()
            # avg = (masked_pop * dis[:self.N_grid]).sum() / self.population.sum()
