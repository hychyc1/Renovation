...
def parse_df_to_env_state(df, village_df):
    ...
    for _, row in village_df.iterrows():
        ...
        env_state['AREA'][r, c] += row.geometry.area
    return env_state
...
def setup_agent():
    ...
    # data_path = 
    villages = gpd.read_file('data_with_id/urban_villages_filtered.shp')
    grid_info = pd.read_csv('data_use/updated_grid_info.csv')
    ...
    villages = villages.reindex(columns=['assign_row', 'assign_col', 'area'])
    print(villages)
